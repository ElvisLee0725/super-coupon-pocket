...
// Guest Login - Login as guest for demo purpose
export const guestLogin = () => async dispatch => {
  ...
  const body = JSON.stringify({ email: 'guest@demo.com', password: 'qaz123' });
  try {
    const res = await axios.post('/api/auth/guest', body, config);
    dispatch({ type: LOGIN_SUCCESS, payload: res.data });
    dispatch(loadUser());
  } catch (err) {
    const errors = err.response.data.error;
